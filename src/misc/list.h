...
#ifndef SRC_WINAPI_LIST_H
#define SRC_WINAPI_LIST_H
...
class List {
 ...
 public:
  List(LIST_ENTRY* head) : m_head(head) {}
  List();

  List(const List& l) : m_head(l.m_head) {}
  List& operator=(const List& l) { return List(l); }
 ...
 public:
  ...
  class iterator {
   ...
   public:
    explicit iterator(LIST_ENTRY* pEntry) : m_current(pEntry) {}
    ...
    T& operator*() const { return entry2item(m_current); }
